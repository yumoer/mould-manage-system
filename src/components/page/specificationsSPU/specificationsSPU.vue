  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-mail"></i> SPU规格
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-button type="primary" icon="el-icon-lx-add" @click="handleAdd(tableData)">增加SPU规格</el-button>
      </div>
      <el-table
        :data="tableData"
        border
...
<script>
  import { getGoodsSpecification,getDeleteSpecification} from '../../../api/index';
  export default {
    data() {
      return {
        query: {
    ...
    methods: {
      ...
      // 新增操作
      handleAdd() {
        this.$router.push({name:'specificationDetails',query:{data:JSON.stringify('')}})
      },
      ...
      handleDel(index, row){
        ...
        this.$confirm('确定要删除吗？', '提示', {
          ...
          .then(() => {
            getDeleteSpecification(row).then(res=>{
